...
import { getUserFeed, getAuthorAvatar, getUserProfile, getStudentProfile,
    addUserFeed
    } from './state.js';
import { escapeHtml, formatFeedTime, getInitials, getFeedGradient, renderAvatar } from './utils.js';
import { addManualTag, getManualTags } from './tags.js';
import { trackInteraction } from './analytics.js';
import { renderStudentDiary } from './Diary.js';
...
export function renderStudentHome() {
    ...
    const feedHtml = getUserFeed().map((post, index) => {
    ...
    container.innerHTML = `
        <section class="mc-page mc-home-page">
            <div class="mc-page-header mc-page-header-row">
                ...
                <button class="mc-btn mc-btn-outline" type="button" data-action="writeDiary">+ Viết Nhật ký</button>
            </div>
...
document.addEventListener("click", (event) => {
    ...
    else if (event.target.closest("[data-action='writeDiary']")) {
        renderStudentDiary();
    }
});